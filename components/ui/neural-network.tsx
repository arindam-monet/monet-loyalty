...
import { AdditiveBlending, BufferAttribute, BufferGeometry, Color, Group, LineBasicMaterial, LineSegments, Points, PointsMaterial, Vector3 } from 'three';
import { useFrame } from '@react-three/fiber';

export default function NeuralNetwork() {
  const groupRef = useRef<Group>();
  const particlesRef = useRef<Points>();
  const linesGeometryRef = useRef<BufferGeometry>();
  ...
  const particlesData: { velocity: Vector3; numConnections: number }[] = useMemo(() => [], []);
  ...
  useEffect(() => {
    ...
    if (particlesRef.current) {
      particlesRef.current.geometry.setAttribute('position', new BufferAttribute(particlePositions, 3));
  ...
  useFrame((_, delta) => {
    ...
    if (linesGeometryRef.current) {
      linesGeometryRef.current.setAttribute('position', new BufferAttribute(positions, 3));
      linesGeometryRef.current.setAttribute('color', new BufferAttribute(colors, 3));
